 ...
 * @author  markstro
 */
public class MmfOuiMultiBasinRunTreeNode extends MmsModelTreeNode implements MmsSingleModelRunner {
...
//    private String[] xrouteVariableIndex;
    ...
    public MmfOuiMultiBasinRunTreeNode(Node xml_node) {
        super(xml_node);
...
//        String mmi = pxml.getElementContent(_xml_node, "@mmi", null);
        ...
        if (mmiNode == null) {
            System.out.println ("MmfOuiMultiBasinRunTreeNode.run: No mmi node found for " + mmi);
            return;
        }
 ...
 */        
        ...
        dataFile = MmsProjectXml.getElementContent(prmsNode, "@dataFileName", null);
 ...
 */
        ...
        String data_file = mmsWorkspace + "/input/data/" + dataFile;
...
//        xrouteVariableName = pxml.getElementContent(routeNode, "@variableName", null); 
...
/*
 *  Run MMF for each subBasin
 */
        for (int i = 0; i < subBasins.length; i++) {
            String datafile = " -set data_file " + relativeMmsWorkspace + "/input/data/" + dataFile;
            ...
            if (variableName != null) {
                statvarfile = " -set nstatVars 1 -set statVar_names " + variableName + " -set statVar_element " + variableIndex + " -set statsON_OFF 1 -set stat_var_file " + statvarfilename;
            }
            String start = " -set start_time " + runStart.getControlFileDateTime();
            String end = " -set end_time " + runEnd.getControlFileDateTime();
            String control = " -C" + relativeMmsWorkspace + "/control/" + subBasins[i] + controlFileExt;
            ...
            System.out.println("MmfOuiMultiBasinRunTreeNode.runModel: " + arg);
...
//        String env = " -por";
            ...
            System.out.println("MmfOuiMultiBasinRunTreeNode.runModel: " + arg);
            if (showRunnerGui) {
                CommandRunner.runModel(arg, "MmfOuiMultiBasinRunTreeNode.runModel: run complete");
            } else {
                CommandRunner.runModel(arg, null);
            }
        }
