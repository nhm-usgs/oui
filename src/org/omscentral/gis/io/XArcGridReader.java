 ...
 */
  ...
  protected Hashtable<String,Object> header = new Hashtable<String,Object>();
 ...
 */
  ...
  private final boolean validKey(String key)
  {
    for (int i = 0; i < KEYS.length; i++) if (KEYS[i].equals(key)) return true;
    return false;
  }

  private final boolean ensureHeader()
  {
    if ((! failed) && (headerLines == 0)) readHeader();
    return (! failed);
  }

  private final boolean ensureBody()
  {
    if ((! failed) && (body == null)) readBody();
    return (! failed);
  }
    ...
    try
    {
          ...
          if (tokens.length == 2)
          {
            ...
            if (validKey(key))
            {
              try
              {
                ...
                else header.put(key, Double.valueOf(tokens[1]));
                headerLines++;
              }
               ...
               || header.containsKey("NODATA_VALUE"))))
      {
        ...
        nbOfRows = ((Integer)header.get("NROWS")).intValue();
         ...
         cellSize = ((Double)header.get("CELLSIZE")).doubleValue();
        missingValue =
            (header.containsKey("NODATA"))
  ...
  protected void readBody()
  {
    if (ensureHeader())
    {
      ...
      try
      {
        ...
        for (int i = 0; i < headerLines; i++) line = reader.readLine();
        ...
        for (int y = 0; y < nbOfRows; y++)
        {
          ...
          if (line != null)
          {
            String[] tokens = Tokenizer.tokenize(line, " \t");
System.err.print(".");
            if (nbOfCols <= tokens.length) for (int x = 0; x < nbOfCols; x++)
            {
              try
              {
                value = Float.valueOf(tokens[x]).floatValue();
...
class Tokenizer extends StringTokenizer {
  ...
  protected static int countRealTokens(String[] tokens, String delimiter) {
    ...
    for (int i = 0; i < tokens.length; i++)
      if ((1 == tokens[i].length()) &&
          (0 <= delimiter.indexOf(tokens[i].charAt(0)))) cnt++;
    return cnt;
  }
  ...
  protected static String getRealToken(String[] tokens, String delim, int idx)  {
	...
	((1 == tokens[i].length()) &&
      ...
      if (cnt == idx) return (isDelimiter) ? "" : tokens[i];
      else if (isDelimiter)
      {
        cnt++;
        if ((cnt == idx) && (i == tokens.length - 1)) return "";
      }
    }
    return null;
  }

  protected static String[] tokenizeStr
    (String string, String delim, boolean complete)  {
    if (string == null) return null;
    else if (string.length() == 0) return new String[0];
    else
    {
  ...
  public static final String join(String[] tokens, String junction) {
    ...
    else
    {
      ...
      if (0 < tokens.length) result += tokens[tokens.length - 1];
