...
public class ThemeFactory {
  ...
  static Map<String,String> readers = new HashMap<String,String>(3);
  static Map<String,String> themes = new HashMap<String,String>(3);
  static Map<String,String[]> fileAssoc= new HashMap<String,String[]>(3);
  static {
  ...
  public static File[] findFiles(String baseDir) throws IOException {
    ...
    for (int i = 0; i < dir.length; i++) {
      ...
      while (it.hasNext()) {
        if (dir[i].toString().endsWith((String)it.next())) {
          files.add(dir[i]);
          break;
        }
      }
  ...
  public static File[] findFiles(String baseDir,String themeName,String ext) throws IOException {
    ...
    for (int i = 0; i < exts.length; i++) {
      file = new File(baseDir + "/" + themeName + exts[i]);
      if (file.exists())
        files.add(file);
    }
    File[] retval = new File[files.size()];
  ...
  public static Theme createTheme(File f) throws IOException  {
    String s = f.toString();
    int i = s.lastIndexOf(f.separator);
  ...
  public static Theme createTheme(String baseDir,String themeName) throws IOException {
    ...
    if (files.length == 0) throw new IOException("Could not read file : " + baseDir  + themeName);
  ...
  private static Theme createFromReaders(Object[] readers) throws IOException {
    ...
    try {
      ...
      if (classToUse.equals(VectorTheme.class.getName())) {
        DbfFileParser dfp = null;
        ShpFileParser sfp = null;
