...
public class CsvReader {
    ...
    final private UserSettings userSettings = new UserSettings();
    private Charset charset = null;
    private boolean useCustomRecordDelimiter = false;
    // this will be our working buffer to hold data chunks
    // read in from the data file
    final private DataBuffer dataBuffer = new DataBuffer();
    final private ColumnBuffer columnBuffer = new ColumnBuffer();
    final private RawRecordBuffer rawBuffer = new RawRecordBuffer();
    private boolean[] isQualified = null;
    private String rawRecord = "";
    final private HeadersHolder headersHolder = new HeadersHolder();
    ...
    private void endColumn() throws IOException {
        ...
        if (startedColumn) {
            if (columnBuffer.Position == 0) {
                if (dataBuffer.ColumnStart < dataBuffer.Position) {
                    ...
                    if (userSettings.TrimWhitespace && !startedWithQualifier) {
                        while (ll >= dataBuffer.ColumnStart
                                && (dataBuffer.Buffer[ll] == Letters.SPACE || dataBuffer.Buffer[ll] == Letters.TAB)) {
                            ll--;
                        }
                    }
                ...
                if (userSettings.TrimWhitespace && !startedWithQualifier) {
                    while (ll >= 0
                            && (columnBuffer.Buffer[ll] == Letters.SPACE || columnBuffer.Buffer[ll] == Letters.SPACE)) {
                        ll--;
                    }
                }
